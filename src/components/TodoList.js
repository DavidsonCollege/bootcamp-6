  ...
  render() {
    return (
      <div>
        ...
        <button onClick={() => this.props.dispatch(changeOrder(Orderings.ALPHABETICAL))}>Order Alphabetically</button>
        ...
        <ol>
          {this.props.todos.filter((item) => !item.completed).map((todo) => {
            ...
            <button onClick={(event) => {
              event.preventDefault();
              this.props.dispatch(changeDate(this.props.dateInput, todo._guid))}}>Change</button>
            </div>
            </li>
          })}
        </ol>
